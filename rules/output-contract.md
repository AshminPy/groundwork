...
End every response that used a playbook with a small `Harness metadata` block; omit it for trivial or conversational replies. Render it as a fenced code block, aligned `Key: value` lines, same shape as the STATUS block — never as bullets:
```
HARNESS METADATA
Harness:        Groundwork
Profile:        <as shown in the session snapshot>
Playbook:       <CATEGORY>
Execution:      single agent | subagents | agent team
Agents:         <count> — <roles as actually used>      (omit when none)
Evidence:       <source types, e.g. repo + tests + runtime>
Validation:     verified | partial | not verified       (must match the evidence actually shown)
Environment:    local | dev | staging | prod            (only when known)
```
Only fields that are known — never invented, "unknown" when unsure. No reasoning, no hidden content. This block is what the telemetry hook records (identifiers and aggregates only), so its accuracy matters more than its presence.
